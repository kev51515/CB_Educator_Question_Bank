...
import {
  ...
  tierBalance,
  useCollegeCatalog,
} from "../../teacher/counseling/collegeAppHelpers";
...
/**
 * BalanceHeader — the reach/target/safety/likely summary with a thin proportion
 * bar and (when unbalanced) a gentle advice line. The student-side mirror of the
 * counselor panel's header; both consume `tierBalance` from collegeAppHelpers.
 */
function BalanceHeader({ apps }: { apps: { tier: Tier | null }[] }) {
  ...
  const classified =
    balance.reach + balance.target + balance.safety + balance.likely;
 ...
 * application has a `college_id`: admit rate, type/size, website, per-plan
 * deadlines, essay prompts, and humanized requirements.
 */
function CatalogDetail({ college }: { college: CatalogCollege | undefined }) {
  if (!college) {
    return (
...
export function StudentCollegeListCard({
  ...
  // Batch-fetch the rich catalog rows for every linked college, so an expanded
  // row can surface admit rate / deadlines / essays / requirements.
  const linkedIds = useMemo(() => apps.map((a) => a.college_id), [apps]);
  ...
  const sortedApps = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      const ad = effectiveDeadline(
        a,
        a.college_id ? catalogById[a.college_id] : undefined,
      );
      const bd = effectiveDeadline(
        b,
        b.college_id ? catalogById[b.college_id] : undefined,
      );
      if (ad && bd) {
  ...
  return (
    <section className="rounded-2xl ring-1 ring-slate-200 dark:ring-slate-800 bg-white/80 dark:bg-slate-900/60 px-5 py-5 space-y-4">
      ...
      {!loading && apps.length > 0 && <BalanceHeader apps={apps} />}
