...
export function TestCompletionModal({ slug, title, onClose }: TestCompletionModalProps) {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    setError(null);
    try {
      const { data, error: rpcError } = await supabase.rpc("test_roster_status", {
        p_slug: slug,
      });
      if (rpcError) {
  ...
  const takenRows = rows.filter((r) => r.run_id !== null);
  const releasedCount = takenRows.filter((r) => r.results_released_at !== null).length;
  const allReleased = takenRows.length > 0 && releasedCount === takenRows.length;
      ...
      >
        <header className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {takenRows.length} of {rows.length} taken · {releasedCount} released
            </p>
          </div>
          <button
            type="button"
        ...
        {loading ? (
          ...
          <p className="rounded-md bg-slate-50 dark:bg-slate-800/60 px-3 py-6 text-center text-sm text-slate-500 dark:text-slate-400">
            No students are assigned this test yet. Add it to a course's Modules
            to assign it.
          </p>
        ) : (
          <ul className="divide-y divide-slate-100 dark:divide-slate-800 rounded-xl ring-1 ring-slate-200 dark:ring-slate-800 overflow-hidden">
            {rows.map((row) => {
              ...
              return (
                <li key={row.student_id} className="flex flex-wrap items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900">
