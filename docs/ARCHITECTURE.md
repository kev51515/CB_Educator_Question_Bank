...
| `journey/` | Gamified Journey view (Khan-style mastery grid; see `docs/JOURNEY_VIEW.md`): pure mastery/points/level logic (`mastery.ts`, `buildJourney.ts`) + `JourneyGrid`/`JourneyHud` presentationals + `TeacherJourneyPanel` (class aggregate). Consumed by `teacher/ModulesPage` (Journey\|List, Journey primary) and `student/StudentCourseView` (behind `STUDENT_JOURNEY_ENABLED`). |
