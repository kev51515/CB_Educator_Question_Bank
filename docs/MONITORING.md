...
- **Deploys and edge** → **Vercel Dashboard**. Build status, runtime logs, cache hit rate.
...
## Vercel Dashboard (Deploy + Edge)

| Tab | Watch for |
|---|---|
| **Deployments** | Latest production deploy succeeded |
| **Logs → Runtime Logs** | Should be near empty — we don't SSR, only static + API proxies |
| **Logs → Build Logs** | Build failures (also delivered to email by default) |
| **Edge Network** | Cache hit rate; very low cache hit means a config bug |
| **Analytics** (paid) | Page views, Core Web Vitals |
...
| Vercel | Deploy failure | Production build fails | Email (built-in) |
...
5. **Vercel → Deployments** — last deploy is green
...
4. **Resolve.** Hotfix → commit → Vercel auto-deploys → confirm the Sentry issue stops getting new events.
...
| Vercel logs | 1 hour (free), 1 day (Pro) |
...
- **Real User Monitoring beyond PostHog.** PostHog gives you clicks and pageviews; for actual performance metrics across the user base, you'd need Sentry Performance at scale or Vercel Speed Insights.
