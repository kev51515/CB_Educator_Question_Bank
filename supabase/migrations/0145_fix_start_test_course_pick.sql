-- =============================================================================
-- Migration: 0145_fix_start_test_course_pick.sql
-- Description: Hotfix to 0143's start_test. The deterministic binding-course
--   tie-break used `min(x)` over a uuid[] (`SELECT min(x) FROM unnest(v_courses) x`)
--   — Postgres has no `min(uuid)` aggregate, so the statement failed at plan time
--   with `function min(uuid) does not exist` for every student whose binding
--   course fell through to the tie-break. Replaced with `ORDER BY x LIMIT 1`.
--
--   Otherwise byte-identical to 0143's start_test. Forward-only (same convention
--   as the 0091 hotfix to 0090). CREATE OR REPLACE — return type unchanged.
-- =============================================================================
...
BEGIN
  ...
  IF NOT v_have THEN
    ...
    ELSE
      ...
      END IF;
      -- FIX: deterministic tie-break via ORDER BY (no min(uuid) aggregate).
...
-- =============================================================================
-- END OF MIGRATION 0145_fix_start_test_course_pick.sql
-- =============================================================================
