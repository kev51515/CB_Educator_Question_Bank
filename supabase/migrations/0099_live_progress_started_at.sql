-- =============================================================================
-- Migration: 0099_live_progress_started_at.sql
-- Description: Add `started_at` to the proctor live-progress view so a teacher
--              can see WHEN each student began the test (alongside the existing
--              current-question / answered / time-left / submitted_at signals).
--
-- The in-progress (and submitted) run already carries started_at on test_runs;
-- this just surfaces it. Adding a column to the RETURNS TABLE changes the
-- function's return type, which CREATE OR REPLACE cannot do — so DROP + CREATE.
-- Body is otherwise identical to 0087's test_live_progress.
--
-- Forward-only.
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    ip.last_seen_at,
    COALESCE(ip.started_at, sub.started_at),  -- when this student began the test
    sub.submitted_at
  FROM assigned a
  LEFT JOIN LATERAL (
...
-- =============================================================================
-- END OF MIGRATION 0099_live_progress_started_at.sql
-- =============================================================================
